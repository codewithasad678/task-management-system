...
<div class="row mx-0">
    <h2 class="primary-bg text-white py-2 text-center">Update Productivity</h1>
    <form action="/productivity/{{$data->id}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row justify-content-evenly my-2 bg-white py-1">
                ...
                <div class="form-group ">
                    ...
                    <select name="task_id" id="task_id" class="form-control">
                        ...
                        @foreach($tasks as $task)
                            <option value="{{$task->id}}" @if($data->task_id == $task->id) {{'selected'}} @endif>{{$task->name}}</option>
                        @endforeach
                    </select>
                    @error('name')
                        <span class="text-danger">{{$message}}</span>
            ...
            <div class="col-sm-6 p-1 px-2 " >
                <div class="form-group ">
                    ...
                    <input type="date" class="form-control" name="date" id="date"  required value="{{$data->date}}">
                    @error('date')
                        <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
                ...
                <div class="row justify-content-evenly">
                    <div class="col-sm-6">
                        <div class="form-group ">
                            ...
                            <input type="time" class="form-control" name="start_time" id="start_time"  required value="{{$data->start_time}}">
                            @error('start_time')
                                <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group ">
                            ...
                            <input type="time" name="end_time" id="end_time" value="{{$data->end_time}}" class="form-control" >
                            @error('end_time')
                                <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
